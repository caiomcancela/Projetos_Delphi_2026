object Form_Login: TForm_Login
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'LOGIN'
  ClientHeight = 141
  ClientWidth = 357
  Color = clSilver
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  Position = poScreenCenter
  TextHeight = 15
  object Imp_Usuario: TcxTextEdit
    Left = 94
    Top = 29
    Properties.CharCase = ecUpperCase
    Properties.OEMConvert = True
    Style.LookAndFeel.NativeStyle = False
    Style.LookAndFeel.SkinName = 'Blue'
    StyleDisabled.LookAndFeel.NativeStyle = False
    StyleDisabled.LookAndFeel.SkinName = 'Blue'
    StyleFocused.LookAndFeel.NativeStyle = False
    StyleFocused.LookAndFeel.SkinName = 'Blue'
    StyleHot.LookAndFeel.NativeStyle = False
    StyleHot.LookAndFeel.SkinName = 'Blue'
    StyleReadOnly.LookAndFeel.NativeStyle = False
    StyleReadOnly.LookAndFeel.SkinName = 'Blue'
    TabOrder = 0
    OnKeyPress = Imp_UsuarioKeyPress
    Width = 233
  end
  object Lbl_Usuario: TcxLabel
    Left = 32
    Top = 33
    Caption = 'USUARIO:'
    Style.LookAndFeel.NativeStyle = False
    Style.LookAndFeel.SkinName = 'GlassOceans'
    StyleDisabled.LookAndFeel.NativeStyle = False
    StyleDisabled.LookAndFeel.SkinName = 'GlassOceans'
    StyleFocused.LookAndFeel.NativeStyle = False
    StyleFocused.LookAndFeel.SkinName = 'GlassOceans'
    StyleHot.LookAndFeel.NativeStyle = False
    StyleHot.LookAndFeel.SkinName = 'GlassOceans'
    TabOrder = 1
  end
  object Imp_Senha: TcxTextEdit
    Left = 94
    Top = 58
    Properties.CharCase = ecUpperCase
    Properties.EchoMode = eemPassword
    Style.BorderStyle = ebs3D
    Style.LookAndFeel.NativeStyle = False
    Style.LookAndFeel.SkinName = 'Blue'
    StyleDisabled.LookAndFeel.NativeStyle = False
    StyleDisabled.LookAndFeel.SkinName = 'Blue'
    StyleFocused.LookAndFeel.NativeStyle = False
    StyleFocused.LookAndFeel.SkinName = 'Blue'
    StyleHot.LookAndFeel.NativeStyle = False
    StyleHot.LookAndFeel.SkinName = 'Blue'
    StyleReadOnly.LookAndFeel.NativeStyle = False
    StyleReadOnly.LookAndFeel.SkinName = 'Blue'
    TabOrder = 2
    OnKeyPress = Imp_SenhaKeyPress
    Width = 233
  end
  object Lbl_Senha: TcxLabel
    Left = 32
    Top = 58
    Caption = 'SENHA:'
    Style.LookAndFeel.NativeStyle = False
    Style.LookAndFeel.SkinName = 'GlassOceans'
    StyleDisabled.LookAndFeel.NativeStyle = False
    StyleDisabled.LookAndFeel.SkinName = 'GlassOceans'
    StyleFocused.LookAndFeel.NativeStyle = False
    StyleFocused.LookAndFeel.SkinName = 'GlassOceans'
    StyleHot.LookAndFeel.NativeStyle = False
    StyleHot.LookAndFeel.SkinName = 'GlassOceans'
    TabOrder = 3
  end
  object Btn_Entrar: TcxButton
    Left = 120
    Top = 93
    Width = 113
    Height = 40
    Caption = 'Entrar'
    LookAndFeel.NativeStyle = False
    LookAndFeel.SkinName = 'Blue'
    ParentShowHint = False
    ShowHint = False
    TabOrder = 4
    OnClick = Btn_EntrarClick
  end
end
